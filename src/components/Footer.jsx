...
const Footer = () => {
  const apiBaseUrl = import.meta.env.VITE_REGUARD_API_WEBSITE_LINK || 'https://reguard-one.vercel.app'
  

  const kycAPIs = [
    { id: 'identity-document-verification', name: 'Identity Document Verification API' },
    { id: 'facial-recognition-match', name: 'Facial Recognition Match API' },
    { id: 'address-verification', name: 'Address Verification API' },
    { id: 'phone-number-validation', name: 'Phone Number Validation API' },
    { id: 'email-verification', name: 'Email Verification API' },
    { id: 'age-verification', name: 'Age Verification API' }
  ]

  const kybAPIs = [
    { id: 'company-registration-lookup', name: 'Company Registration Lookup API' },
    { id: 'ultimate-beneficial-owner', name: 'Ultimate Beneficial Owner API' }
  ]
  ...
  {/* Second Row - API Links */}
        <div className="border-t mt-12 border-slate-700 pt-12">
          <div className="grid lg:grid-cols-4 gap-x-14 gap-y-12">
            ...
            {/* KYC APIs */}
            <div>
              <h3 className="text-lg font-semibold mb-8">KYC APIs</h3>
              <ul className="space-y-4">
                {kycAPIs.map((api) => (
                  <li key={api.id}>
                    <a href={`${apiBaseUrl}/apis/kyc/${api.id}`} target="_blank" className="text-slate-400 hover:text-white transition-colors text-sm">
                      {api.name}
                    </a>
                  </li>
                ))}
            ...
            {/* KYB APIs */}
            <div>
              <h3 className="text-lg font-semibold mb-8">KYB APIs</h3>
              <ul className="space-y-4">
                {kybAPIs.map((api) => (
                  <li key={api.id}>
                    <a href={`${apiBaseUrl}/apis/kyb/${api.id}`} target="_blank" className="text-slate-400 hover:text-white transition-colors text-sm">
                      {api.name}
                    </a>
                  </li>
                ))}
            ...
            {/* AML APIs */}
            <div>
              <h3 className="text-lg font-semibold mb-8">AML APIs</h3>
              <ul className="space-y-4">
                {amlAPIs.map((api) => (
                  <li key={api.id}>
                    <a href={`${apiBaseUrl}/apis/aml/${api.id}`} target="_blank" className="text-slate-400 hover:text-white transition-colors text-sm">
                      {api.name}
                    </a>
                  </li>
                ))}
            ...
            {/* All APIs Link */}
            <div>
              <h3 className="text-lg font-semibold mb-8">All Solutions</h3>
              <div>
                <a href={`${apiBaseUrl}/apis`} target="_blank" className="text-red-500 hover:text-red-400 transition-colors font-semibold text-base">
                  Check out all API solutions →
                </a>
              </div>
            </div>
